...
function PixiContent() {
  return (
    <Pixi
      canvasAttrs={{
        className: "cw-[1314px] h-506 absolute top-100 lg-max:hidden pointer-events-none",
      }}
      fps={Infinity}
...
export default function HomeHeroPixi() {
  ...
  if (hasError) {
    ...
    return <div className="cw-[1314px] h-506 absolute top-100 lg-max:hidden pointer-events-none" />;
  }
  
  return (
    <Suspense fallback={<div className="cw-[1314px] h-506 absolute top-100 lg-max:hidden pointer-events-none" />}>
      <PixiContent />
    </Suspense>
  );
}
