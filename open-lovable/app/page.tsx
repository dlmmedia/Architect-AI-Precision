...
export default function HomePage() {
  ...
  const handleSubmit = async (selectedResult?: SearchResult) => {
    ...
    console.log('[HomePage] handleSubmit called', { 
      ...
      selectedResult,
      urlState: url,
      urlStateLength: url.length
    });
    
    if (!inputValue) {
      console.log('[HomePage] No input value, showing error');
      toast.error(generationMode === 'prompt' ? "Please describe what you want to build" : "Please enter a URL or search term");
      return;
    }
    
    // If in prompt mode, skip URL/search logic and go straight to generation
    if (generationMode === 'prompt') {
      console.log('[HomePage] Prompt mode detected, setting up session storage and navigating...');
      sessionStorage.setItem('generationMode', 'prompt');
      sessionStorage.setItem('textPrompt', inputValue);
      sessionStorage.setItem('selectedStyle', selectedStyle);
      sessionStorage.setItem('selectedModel', selectedModel);
      sessionStorage.setItem('autoStart', 'true');
      console.log('[HomePage] Session storage set:', {
        generationMode: sessionStorage.getItem('generationMode'),
        textPrompt: sessionStorage.getItem('textPrompt'),
        autoStart: sessionStorage.getItem('autoStart')
      });
      console.log('[HomePage] Calling router.push to /generation');
      router.push('/generation');
      return;
    }
    
    // Clone mode logic below
    ...
    if (selectedResult) {
      ...
      setTimeout(() => {
        sessionStorage.setItem('generationMode', 'clone');
      ...
      <div className="min-h-screen bg-background-base">
          ...
          <HeaderWrapper>
            <div className="max-w-[900px] mx-auto w-full flex justify-between items-center">
              ...
              <div className="flex gap-8">
                <a
                  className="contents"
                  href="https://github.com/dlmmedia/architect"
                  target="_blank"
                >
        ...
        <section className="overflow-x-clip" id="home-hero">
          <div className="pt-28 lg:pt-254 lg:-mt-100 pb-115 relative" id="hero-content">
            ...
            <div className="relative container px-16">
              ...
              <p className="text-center text-body-large">
                Build and reimagine websites with AI precision.
              </p>
              <Link
          ...
          <div className="container lg:contents !p-16 relative -mt-90">
            ...
            <div className="max-w-552 mx-auto z-[200] relative">
              <div className="rounded-20 -mt-30 lg:-mt-30 relative z-[200]">
                {/* Mode Toggle */}
                <div className="mb-12 flex justify-center relative z-[200]">
                  <div className="inline-flex bg-white rounded-12 p-4 border border-gray-200 relative z-[200]" style={{
                    ...
                    <button
                      ...
                      className={`
                        px-16 py-8 rounded-8 text-label-medium font-medium transition-all cursor-pointer relative z-[201]
                    ...
                    <button
                      onClick={() => {
                        console.log('[HomePage] Build from Scratch clicked');
                ...
                <div className="p-16 flex gap-12 items-center w-full relative bg-white rounded-20">
                  ...
                  {hasSearched && searchResults.length > 0 && !isFadingOut ? (
                    ...
                    <>
                      {generationMode === 'prompt' ? (
                        // Blueprint icon for prompt mode
                        <svg 
                          width="20" 
                          height="20" 
                          viewBox="0 0 20 20" 
                          ...
                          className="opacity-40 flex-shrink-0"
                        >
                          <path d="M4 4H16M4 4V16M4 4L16 16M16 4V16M16 16H4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                      ...
                      <input
                        ...
                        placeholder={generationMode === 'prompt' ? "Describe what you want to build..." : "Enter URL or search term..."}
                        type="text"
                        value={url}
                        disabled={isSearching}
                        onChange={(e) => {
                          const value = e.target.value;
                          console.log('[HomePage] Input onChange', { value, generationMode });
                          setUrl(value);
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && !isSearching) {
                            e.preventDefault();
                            // Ensure we have the latest value from the input
                        ...
                        onFocus={() => {
                          if (generationMode === 'clone' && url.trim() && !isURL(url) && searchResults.length > 0) {
                            setShowSearchTiles(true);
                          }
                        }}
                      />
                      ...
                      >
                        <HeroInputSubmitButton 
                          dirty={url.length > 0} 
                          buttonText={generationMode === 'prompt' ? 'Start Building' : (isURL(url) ? 'Scrape Site' : 'Search')} 
                          disabled={isSearching}
                        />
                      </div>
                    </>
                ...
                {/* Options Section - Only show when valid URL in clone mode */}
                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
                  (generationMode === 'clone' && isValidUrl) || generationMode === 'prompt' ? 'max-h-[200px] opacity-100' : 'max-h-0 opacity-0'
                }`}>
                  <div className="p-[28px]">
                    <div className="border-t border-gray-100 bg-white">
                      {/* Style Selector */}
                      <div className={`mb-2 pt-4 transition-all duration-300 transform ${
                        (generationMode === 'clone' && isValidUrl) || generationMode === 'prompt' ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'
                          ...
                          {styles.map((style, index) => (
                            <button
                              ...
                              className={`
                                ...
                                ${(generationMode === 'clone' && isValidUrl) || generationMode === 'prompt' ? 'opacity-100' : 'opacity-0'}
                              `}
                              style={{
        ...
        {/* Full-width oval carousel section - only show in clone mode */}
        {generationMode === 'clone' && showSearchTiles && hasSearched && (
