...
function AISandboxPage() {
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      const textPrompt = sessionStorage.getItem('textPrompt');
      
      if (storedUrl || textPrompt) {
        // Mark that we have an initial submission since we're loading with a URL or prompt
        ...
        setHomeUrlInput(storedUrl || textPrompt || '');
...
//   };
  ...
  const startGeneration = async () => {
    ...
    // Check if this is prompt mode or clone mode
    const generationMode = sessionStorage.getItem('generationMode') || 'clone';
    ...
    if (isPromptMode) {
      addChatMessage(`Starting to build from your description...`, 'system');
    } else {
    ...
    // Only capture screenshot for clone mode
    if (!isPromptMode) {
      let displayUrl = homeUrlInput.trim();
      if (!displayUrl.match(/^https?:\/\//i)) {
        displayUrl = 'https://' + displayUrl;
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (isPromptMode) {
          // Prompt mode: Skip scraping and use text prompt directly
          setUrlStatus(['Analyzing your requirements...']);
          addChatMessage('Understanding your requirements...', 'system');
          ...
          setConversationContext(prev => ({
            ...prev,
            currentProject: 'Custom Build from Prompt'
          }));
          
          // Build prompt from user's description
          const userPrompt = homeUrlInput.trim();
          let styleContext = homeContextInput ? homeContextInput.trim() : '';
          
          prompt = `I want to build a complete React application based on this description:

${userPrompt}

${styleContext ? `DESIGN STYLE/ADDITIONAL REQUIREMENTS:
${styleContext}
...
- Implement ALL features described in the requirements
- Use Tailwind CSS for all styling (no custom CSS files)
- Make it responsive and modern
- Create proper component structure
- Make sure the app actually renders visible content
- Create ALL components that you reference in imports
${styleContext ? '- Apply the design style/requirements throughout the application' : ''}

Focus on creating a clean, modern, and fully functional application.`;
        } else {
          // Clone mode: Scrape website first
...
Focus on the key sections and content, making it clean and modern.`;
        ...
        // Add message to chat
        addChatMessage(homeUrlInput, 'user', isPromptMode ? {} : {
          scrapedUrl: homeUrlInput.match(/^https?:\/\//i) ? homeUrlInput : 'https://' + homeUrlInput,
